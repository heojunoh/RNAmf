...
ALMC_two_level_optm <- function(Xref, fit, cost, funcs){
  ...
  xoptm1 <- xoptm2 <- matrix(NA, nrow=nrow(newx), ncol=ncol(newx))
  value1 <- value2 <- rep(NA, nrow(newx))


  ### Optimize at level 1 ###
    out <- optim(newx, obj.ALC_two_level_1, method="L-BFGS-B", lower=0, upper=1, fit=fit, Xref=Xref)
    xoptm1 <- out$par
    value1 <- out$value

  ### Optimize at level 2 ###
    out <- optim(newx, obj.ALC_two_level_2, method="L-BFGS-B", lower=0, upper=1, fit=fit, Xref=Xref)
    xoptm2 <- out$par
    value2 <- out$value

  ### Select the next level and point ###
  ALMCvalue <- c(Icurrent - value1, Icurrent - value2)/c(cost[1], cost[1]+cost[2])

  chosen <- list("level"=which.max(ALMCvalue), # next level
                 "location"=which.max(predsig2), # next location
                 "Xnext"=Xcand[which.max(predsig2),]) # next point
